...
import type { UnifiedProcesso } from "@/lib/societario/dataSource";
...
export function ProcessoForm({
  ...
  const [atividades, setAtividades] = useState<Atividade[]>(() => {
    if (initial && initial.activities.length > 0) {
      return initial.activities.map((a) => ({
        id: a.id,
        nome: a.name,
      ...
      <section className="bg-white border border-gray-200 rounded-lg p-5 space-y-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <Field label="CNPJ / CPF">
            <input
              name="inscription"
              defaultValue={initial?.inscription || ""}
              className="form-input"
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          <Field label="Tipo de processo *" required>
            <select
              name="process"
              required
              defaultValue={initial?.process || tipos[0] || ""}
              className="form-input"
            >
              {tipos.map((t) => (
          ...
          <Field label="Status">
            <select
              name="status"
              defaultValue={initial?.status || "ACOMPANHAMENTO"}
              className="form-input"
            >
          ...
          <Field label="Data de início">
            <input
              ...
              className="form-input"
              defaultValue={
                initial?.started_in || new Date().toISOString().slice(0, 10)
              }
            />
          </Field>
          <Field label="Valor (R$)">
          ...
          <Field label="Proposta">
            <input
              name="proposal"
              defaultValue={initial?.proposal || ""}
              className="form-input"
              placeholder="000000.2026"
            />
          </Field>
        </div>
