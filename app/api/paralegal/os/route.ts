...
import { listarOS, criarOS, type OS } from "@/lib/paralegal/os";
import { GraphErro } from "@/lib/paralegal/graph";

// Paralegal · Controle — os. Migrado do Paralegal System.
...
function validarCorpo(body: Partial<OS>): string | null {
...
export async function POST(req: Request) {
  ...
  try {
    const { id: _id, ...dados } = body as OS;
    const item = await criarOS(dados);
