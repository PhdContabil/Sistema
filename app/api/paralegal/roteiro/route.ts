...
import { listarRoteiro, criarEtapa, type EtapaRoteiro } from "@/lib/paralegal/roteiro";
import { GraphErro } from "@/lib/paralegal/graph";

// Paralegal · Controle — roteiro. Migrado do Paralegal System.
...
function validarCorpo(body: Partial<EtapaRoteiro>): string | null {
...
export async function GET() {
  ...
  try {
    const itens = await listarRoteiro();
...
export async function POST(req: Request) {
  ...
  try {
    const { id: _id, ...dados } = body as EtapaRoteiro;
    const item = await criarEtapa(dados);
