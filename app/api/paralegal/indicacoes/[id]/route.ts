...
import { obterIndicacao, atualizarIndicacao, excluirIndicacao, type Indicacao } from "@/lib/paralegal/indicacoes";
...
export async function GET(_req: Request, { params }: { params: { id: string } }) {
  ...
  try {
    const item = await obterIndicacao(params.id);
...
export async function PATCH(req: Request, { params }: { params: { id: string } }) {
  ...
  try {
    await atualizarIndicacao(params.id, body);
...
export async function DELETE(_req: Request, { params }: { params: { id: string } }) {
  if (!(await souAutorizado())) return NextResponse.json({ error: "Sem acesso ao Paralegal." }, { status: 403 });
  try {
    await excluirIndicacao(params.id);
    return NextResponse.json({ ok: true });
  } catch (e) {
    const status = e instanceof GraphErro ? e.status : 502;
