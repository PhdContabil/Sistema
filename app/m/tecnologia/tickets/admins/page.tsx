...
import TicketsAdmins from "@/components/apps/TicketsAdmins";
...
export default async function Page() {
  ...
  return (
    <Workspace moduleId="tecnologia" appName="Tickets · Gerenciar admins">
      <TicketsShell resumo={resumo} souAdmin={souAdmin} souAdminGeral={souAdminGeral} meuSetor={meuSetor}>
        <div className="mb-4">
          <h1 className="text-2xl font-bold text-slate-900 dark:text-white">Gerenciar admins</h1>
          <div className="text-sm text-slate-500 dark:text-slate-400 mt-1">
            Quem edita horas/valores dos tickets e acessa as telas de administração.
          </div>
        </div>
        {!souAdmin ? (
          ...
          <TicketsAdmins meuEmail={meuEmail} />
        )}
      </TicketsShell>
    </Workspace>
  );
